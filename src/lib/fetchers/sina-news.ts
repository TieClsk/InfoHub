...
const SOURCE_ID = 'sina';

export async function fetchSinaNews(): Promise<FetcherResult<RawContentInput>> {
  ...
  try {
    ...
    const json = (await response.json()) as {
      result: { data: Array<{ title: string; url: string; intro: string; ctime: string; media_name: string }> };
    };

    const items = json.result?.data || [];
    const rawItems: RawContentInput[] = items.map((item, i) => ({
      sourceId: SOURCE_ID,
      externalId: item.url?.split('/').pop()?.slice(0, 50) || `sina-${i}`,
